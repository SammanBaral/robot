    ...
    while global_state.running:
        if not global_state.nlp:
            ...
            if command:
                ...
                if command == "nitro":  # Check for the specific command 'hello'
                    print("Detected command 'nitro'")
                    add_wheel_command(0,0)
                    # global_state.hello_detected = False  # Reset the state
                    handle_user_interaction()
                    # global_state.hello_detected = False  # Reset the state after handling
