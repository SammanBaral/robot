...
def say_turning():
  ...
  # Adjust the sleep time as needed for a 180
...
def hand_gesture_recognition_thread():
    # global global_state
    ...
    #         frame_rgb = cv2.cvtColor(global_state.frame2, cv2.COLOR_BGR2RGB)
    ...
    #                 if index_is_open and middle_is_open and ring_is_open and pinky_is_open and thumb_is_open:
    #                     if not global_state.hello_detected:
    #                         print("Hello detected!")
    #                         global_state.hello_detected = True
    #                         add_wheel_command(0, 0)  # Stop the robot when hello gesture is detected
    #                         handle_user_interaction()  # Start user interaction
    #                 else:
    #                     global_state.hello_detected = False

    #         cv2.imshow('Hand Gesture Recognition', global_state.frame)

    #         if cv2.waitKey(1) & 0xFF == ord('q'):
    #             break
    main(global_state.arduino)
...
if __name__ == "__main__":
    try:
        ...
        wheel_command_thread = threading.Thread(target=send_wheel_command_thread)
        wheel_command_thread.start()
        ...
        hand_gesture_thread = threading.Thread(target=hand_gesture_recognition_thread)
